...
Style = (function() {
  ...
  Style.prototype.joinSelectors = function(directSelector, imageSelector) {
    ...
    ret = (function() {
      return imageSelector.split(',');
    })();

    return ret.join(',');
  };

  Style.prototype.resolveImageSelector = function(name, fullpath, selector) {
    name = name.split("@4x").join("");
    name = name.split("@2x").join("");

    if(name.indexOf('-on') !== -1) {
      name = selector + ":hover ." + name.split("-on").join("");
    } else {
      name = selector + " ." + name;
    }
  ...
  Style.prototype.generate = function(options) {
    ...
    styles = [this.css(this.selector + " .sprite", ["  background: url(" + relativeImagePath + ") no-repeat", "  background-size: " + (width / pixelRatio) + "px " + (height / pixelRatio) + "px "])];
    for (_i = 0, _len = images.length; _i < _len; _i++) {
      ...
      image.selector = this.resolveImageSelector(image.name, image.path, this.selector);
